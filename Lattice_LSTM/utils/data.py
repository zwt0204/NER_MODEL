...
class Data(object):
    def __init__(self): 
        ...
        self.gaz = Gazetteer(self.gaz_lower)  # trie
        # map gaz to id
    ...
    def build_gaz_file(self, gaz_file):
        ...
        if gaz_file:
            ...
            for fin in fins:
                ...
                if fin:
                    self.gaz.insert(fin, "one_source")  # 构建原始的键树词典，一般来说词典需要足够大
        ...
        for line in in_lines:
            if len(line) > 3:
                ...
                for idx in range(w_length):
                    matched_entity = self.gaz.enumerateMatchList(word_list[idx:])  # 词汇前缀匹配
                    for entity in matched_entity: # 将键树匹配的结果加入词典
    ...
    def build_biword_pretrain_emb(self, emb_path):
        ...
        pretrain_biword_embedding, self.biword_emb_dim = build_pretrain_embedding(emb_path,
                                                                                       self.biword_alphabet,
                                                                                       self.biword_emb_dim,
                                                                                       self.norm_biword_emb)
        return pretrain_biword_embedding

    def build_gaz_pretrain_emb(self, emb_path):
        print("build gaz pretrain emb...")
        self.pretrain_gaz_embedding, self.gaz_emb_dim = build_pretrain_embedding(emb_path,
                                                                                 self.gaz_alphabet,
                                                                                 self.gaz_emb_dim,
                                                                                 self.norm_gaz_emb)
    ...
    def generate_instance(self, input_file, name):
        self.fix_alphabet()
        if name == "train":
            self.train_texts, self.train_Ids = read_seg_instance(input_file, self.word_alphabet,
                                                                 self.biword_alphabet, self.char_alphabet,
                                                                 self.label_alphabet, self.number_normalized,
                                                                 self.MAX_SENTENCE_LENGTH)
        elif name == "dev":
            self.dev_texts, self.dev_Ids = read_seg_instance(input_file, self.word_alphabet,
                                                             self.biword_alphabet, self.char_alphabet,
                                                             self.label_alphabet, self.number_normalized,
                                                             self.MAX_SENTENCE_LENGTH)
        elif name == "test":
            self.test_texts, self.test_Ids = read_seg_instance(input_file, self.word_alphabet,
                                                               self.biword_alphabet, self.char_alphabet,
                                                               self.label_alphabet, self.number_normalized,
                                                               self.MAX_SENTENCE_LENGTH)
        elif name == "raw":
            self.raw_texts, self.raw_Ids = read_seg_instance(input_file, self.word_alphabet,
                                                             self.biword_alphabet, self.char_alphabet,
                                                             self.label_alphabet, self.number_normalized,
                                                             self.MAX_SENTENCE_LENGTH)
        else:
    ...
    def generate_sentence_instance_with_gaz(self, sentence):
        self.fix_alphabet()
        texts, Ids = read_instance_with_gaz_for_sentence(sentence, self.gaz, self.word_alphabet,
                                                        self.biword_alphabet, self.char_alphabet,
                                                        self.gaz_alphabet, self.number_normalized,
                                                        self.MAX_SENTENCE_LENGTH)
        return texts, Ids
    ...
    def write_decoded_results(self, output_file, predict_results, name):
        ...
        if name == 'raw':
            ...
            print("Error: illegal name during writing predict result, "
                  "name should be within train/dev/test/raw !")
